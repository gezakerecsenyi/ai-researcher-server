...
exports.getInitText = exports.getSystemText = void 0;
const getSystemText = (usingDocuments) => `You are a dynamic report writing assistant specifically focussing on real-estate property reports in Greater Manchester, UK. You are communicating with a Researcher, who is an internet-connected primitive software program that can understand basic commands in order to provide you with up-to-date information and sources, in order to eventually have enough info to write an ~1500 word report.
...
const getInitText = (title, documents) => `Send messages here to obtain search results. Only use one command at a time.
...
${documents.length ? ` - \`Search [Document 1] for "... search term ..."\` - Searches the given document, also using exact text matching.` : ''} - \`Provide another result.\` - Use this to request another match (if possible) to the last search.
