...
async function getReports(title, documents, initialSearch, reportCount = 1, callLimit = (reportCount + 1) * 30) {
    let attempts = 0;
    let totalCalls = 0;
    try {
        while (attempts < 5) {
            ...
            async function evaluateState(state) {
                ...
                const reportedState = {
                    ...state,
                    lastReport: await (0, getCompletion_1.default)(title, documents, chatSoFar, matchesForThis, lastResponse, () => {
                        totalCalls++;
